...
clear
%% 1、载入数据
rawdata = convertTDMS(1,'D:\LOT\LOT_DATA\2024_4_26\test3.tdms'); 
for i=1:10
...
data=data1(fs+1:(result-1)*fs,1:10);     %丢弃第一秒和最后一秒数据
Gvo=smoothdata(data(:,9));                  %振镜反馈电压
RS=smoothdata(data(:,10));                  %共振振镜反馈电压
x=1:length(RS);
figure,
% plot(x,Gvo,x,RS,'-g');
plot(x,Gvo);
...
[Gvo_pks,Gvopks_locs] = findpeaks(Gvo, 'MinPeakHeight', 1.3, 'MinPeakDistance', (1/slow_fs)*fs);
...
singleframepos=RS_locs(RS_locs < Gvoval_locs(2) & RS_locs > Gvopks_locs(2));
clear framedata
for i=1:length(singleframepos)
    % framedata((i-1)*320+1:320*i-160,1:8)=flip(data(singleframepos(i)+20:singleframepos(i)+179,1:8),1);
    framedata((i-1)*360+1:360*i-180,1:8)=data(singleframepos(i)-89:singleframepos(i)+90,1:8);
    framedata(360*i-179:360*i,1:8)=flip(data(singleframepos(i)+111:singleframepos(i)+290,1:8),1);
end

framedata=reshape(framedata,180,[],8);
% framedata=permute(framedata, [2 1 3]);
...
timeSeriesData=zeros(180,frameline,8,frame);
for i=1:length(Gvopks_locs)
    ...
    % 帧数据处理
    clear framedata
    framedata=zeros(180*frameline,8);
    for j=1:length(singleframepos)
        % framedata((i-1)*320+1:320*i-160,1:8)=flip(data(singleframepos(i)+20:singleframepos(i)+179,1:8),1);
        framedata((j-1)*360+1:360*j-180,1:8)=flip(data(singleframepos(j)-89:singleframepos(j)+90,1:8),1);
        framedata(360*j-179:360*j,1:8)=data(singleframepos(j)+111:singleframepos(j)+290,1:8);
    end
